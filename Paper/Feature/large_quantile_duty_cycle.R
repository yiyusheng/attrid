...
#
# Description: generate duty cycle quantile from 98% to 100% binned by 0.1% for each disks
#
...
large_quantile_dutycycle<- function(i){
  ...
  r <- lapplyX(splitDT,function(df)quantile(df$util,seq(0.98,1,0.001)))
...
names(r) <- c('svrid',paste('X',980:1000,sep = ''))
save(r,file = file.path(dir_data,'large_quantile_dutycycle.Rda'))
